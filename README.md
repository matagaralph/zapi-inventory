...
- `config` - Required - A plain JavaScript object that contains the configuration options.
  ...
  - `store` - Optional - A custom token store implementation. The SDK uses an in-memory store by default. Provide your own `TokenStore` implementation (file, database, Redis, etc.) to persist tokens across process restarts.
